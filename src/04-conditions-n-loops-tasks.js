...
function doRectanglesOverlap(rect1, rect2) {
  const hOvr = (rect1.left + rect1.width >= rect2.left) && (rect2.left + rect2.width >= rect1.left);
  const vOvr = (rect1.top + rect1.height >= rect2.top) && (rect2.top + rect2.height >= rect1.top);
  return hOvr && vOvr;
}
...
function isCreditCardNumber(ccn) {
  ...
  let sum = 0;
  /* eslint-disable-next-line no-plusplus */
  for (let i = stringCCN.length - 1; i >= 0; i--) {
    const digit = parseInt(stringCCN[i], 10);
...
function getCommonDirectoryPath(paths) {
  ...
  for (let i = 1; i < paths.length; i++) {
    ...
    while (j < commonPath.length && commonPath[j] === filename[j]) {
      /* eslint-disable-next-line no-plusplus */
      j++;
    }
...
function evaluateTicTacToePosition(position) {
  ...
  /* eslint-disable-next-line no-plusplus */
  for (let j = 0; j < 3; j++) {
    if (position[0][j] === position[1][j] && position[1][j] === position[2][j] && position[0][j]) {
      return position[0][j];
    }
  }

  if (
    (position[0][0] === position[1][1] && position[1][1] === position[2][2] && position[0][0])
    || (position[0][2] === position[1][1] && position[1][1] === position[2][0] && position[0][2])
  ) {
    return position[1][1];
  }
